...
def create_favorites_planets(planet_id):
    ...
    db.session.commit()
    return jsonify('Planet has been added to your Favorites!'), 201
...
def create_favorites_character(character_id):
    ...
    db.session.commit()
    return jsonify('Character has been added to your Favorites!'), 201
...
def delete_favorites_planet(user_id, planet_id):
    ...
    if favorite_user_planet is None:
        return jsonify('Could not find your favorite Planet!'), 404
    db.session.delete(favorite_user_planet)
    db.session.commit()
    return jsonify('Successfully deleted your favorite Planet!'), 200
...
def delete_favorites_character(user_id, character_id):
    ...
    if favorite_user_character is None:
        return jsonify('Could not find your favorite Character!'), 404
    db.session.delete(favorite_user_character)
    db.session.commit()
    return jsonify('Successfully deleted your favorite Character!'), 200
